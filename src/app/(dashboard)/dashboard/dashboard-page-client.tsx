...
import { getSalesByPeriod, getSalesByBeverage, getTransactions } from '@/lib/api/analytics.api';
import { ApiError } from '@/lib/api/api-client';
import type { SalesByPeriodResponse, SalesByBeverageResponse, TransactionsResponse } from '@/types/analytics.types';
...
export function DashboardPageClient() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [periodRes, monthBevRes, weekBevRes, dayBevRes, transRes] = await Promise.allSettled([
        getSalesByPeriod(),
        getSalesByBeverage(),
      ...
      const results = [periodRes, monthBevRes, weekBevRes, dayBevRes, transRes];
    ...
    <div className="mx-auto min-w-0 max-w-[1400px] space-y-6 animate-fadeIn">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--text-muted)]">Resumen de ventas — Los Amigos</p>
        </div>
